...
if not database_url:
    ...
    print("例: export DATABASE_URL='postgresql://username:password@host:port/dbname'")
    sys.exit(1)

print(f"データベースURL: {database_url.split('@')[0]}@***")  # パスワードを隠す
...
with app.app_context():
    ...
    if existing_count > 0:
        print(f"既存の問題データが{existing_count}問見つかりました。")
        # 非対話モード: 既存データを削除して再投入
